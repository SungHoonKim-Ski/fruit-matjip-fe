...
import { cancelCourierPayment } from '../../utils/api';
import { safeErrorLog } from '../../utils/environment';

export default function CourierCancelPage() {
  ...
  useEffect(() => {
    ...
    const cancel = async () => {
      try {
        const res = await cancelCourierPayment(orderCode);
        if (!res.ok) throw new Error('cancel failed');
        setStatus('success');
      } catch (e) {
        safeErrorLog(e, 'CourierCancelPage - cancel');
        setStatus('error');
      }
    };
      ...
      <div className="bg-white border rounded-lg shadow-sm p-6 w-full max-w-sm text-center">
        ...
        {status === 'success' && (
          <>
            ...
            <p className="mt-2 text-sm text-gray-600">장바구니에서 다시 주문할 수 있습니다.</p>
            <button
              type="button"
